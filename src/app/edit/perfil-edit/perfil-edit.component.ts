...
export class PerfilEditComponent implements OnInit {
  ...
  altUser() {
    ...
    if (this.usuario.nome == null || this.usuario.usuario == null || this.usuario.senha == null) {
      ...
      this.alertas.showAlertDanger("Mínimo de caracteres na senha é 6")
    } else {
      if (this.confirmarSenha == this.usuario.senha) {
        ...
        this.authService.putUser(this.usuario).subscribe((resp: Usuario) => {
          this.usuario = resp;
          this.alertas.showAlertSuccess('Perfil alterado com sucesso! Entre novamente');
          this.router.navigate(['/entrar']);
          environment.token = ''
          environment.nome = ''
